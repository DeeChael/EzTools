package lib.ezt.nms.v1_13_R1.commands;
...
public final class CommandEzTItem {
    ...
    public static void register(com.mojang.brigadier.CommandDispatcher<CommandListenerWrapper> commandDispatcher) {
        ...
        LiteralArgumentBuilder<CommandListenerWrapper> command = CommandDispatcher
                .a("ezt-item")
                .requires((requirement) -> requirement.hasPermission(4) && requirement.getBukkitSender().hasPermission("ezt.command.ezt-item"));
        //Name command
        command.then(CommandDispatcher
                .a("name")
                .requires((requirement) -> requirement.hasPermission(4) && requirement.getBukkitSender().hasPermission("ezt.command.ezt-item.name"))
        ...
        command.then(CommandDispatcher
                .a("lore")
                .requires((requirement) -> requirement.hasPermission(4) && requirement.getBukkitSender().hasPermission("ezt.command.ezt-item.lore"))
                .then(CommandDispatcher
                        .a("content", ArgumentChat.a())
                        .executes((commandContext -> executeLore(commandContext.getSource(), commandContext.getSource().h(), ArgumentChat.a(commandContext, "content"))))
                )
        );
        //Enchant command
        command.then(CommandDispatcher
                .a("enchant")
                .requires((requirement) -> requirement.hasPermission(4) && requirement.getBukkitSender().hasPermission("ezt.command.ezt-item.enchant"))
                .then(CommandDispatcher
        ...
        LiteralArgumentBuilder<CommandListenerWrapper> attributeCommand =  CommandDispatcher
                .a("attribute")
                .requires((requirement) -> requirement.hasPermission(4) && requirement.getBukkitSender().hasPermission("ezt.command.ezt-item.attribute"));
        for (Attribute attribute : Attribute.values()) {
            ...
            for (EquipmentSlot equipmentSlot : EquipmentSlot.values()) {
                subAttribute.then(CommandDispatcher
                        ...
                        .then(CommandDispatcher
                                .a("add")
                                .requires((requirement) -> requirement.hasPermission(4) && requirement.getBukkitSender().hasPermission("ezt.command.ezt-item.attribute.add"))
                                .then(CommandDispatcher
                        ...
                        .then(CommandDispatcher
                                .a("set")
                                .requires((requirement) -> requirement.hasPermission(4) && requirement.getBukkitSender().hasPermission("ezt.command.ezt-item.attribute.set"))
                                .then(CommandDispatcher
                                        .a("amount", DoubleArgumentType.doubleArg())
                        ...
                        .then(CommandDispatcher
                                .a("remove")
                                .requires((requirement) -> requirement.hasPermission(4) && requirement.getBukkitSender().hasPermission("ezt.command.ezt-item.attribute.remove"))
                        ...
                        .then(CommandDispatcher
                                .a("get")
                                .requires((requirement) -> requirement.hasPermission(4) && requirement.getBukkitSender().hasPermission("ezt.command.ezt-item.attribute.get"))
        ...
        command.then(CommandDispatcher
                .a("unbreakable")
                .requires((requirement) -> requirement.hasPermission(4) && requirement.getBukkitSender().hasPermission("ezt.command.ezt-item.unbreakable"))
    ...
    private static int executeLore(CommandListenerWrapper commandListenerWrapper, EntityPlayer entityPlayer, IChatBaseComponent iChatBaseComponent) {
        int i = 0;
        if (isJsonArray(iChatBaseComponent.getText())) {
            JsonArray jsonArray = (new JsonParser()).parse(iChatBaseComponent.getText()).getAsJsonArray();
