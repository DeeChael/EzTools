...
public class CommandEzT {
    ...
    public static void register(com.mojang.brigadier.CommandDispatcher<CommandListenerWrapper> commandDispatcher) {
        LiteralCommandNode<CommandListenerWrapper> literalCommandNode = commandDispatcher.register(
                CommandDispatcher
                        .a("ezt")
                        .requires((requirement) -> requirement.hasPermission(4) && requirement.getBukkitSender().hasPermission("ezt.command.ezt"))
                        .then(
                                CommandDispatcher
                                        .a("reload")
                                        .requires((requirement) -> requirement.hasPermission(4) && requirement.getBukkitSender().hasPermission("ezt.command.ezt.reload"))
                                        .executes((commandContext -> executeReload(commandContext.getSource())))
                        )
                        .then(
                                CommandDispatcher
                                        .a("help")
                                        .requires((requirement) -> requirement.hasPermission(4) && requirement.getBukkitSender().hasPermission("ezt.command.ezt.help"))
                                        .executes((commandContext -> executeHelp(commandContext.getSource())))
                        )
                        .then(
                                CommandDispatcher
                                        .a("info")
                                        .requires((requirement) -> requirement.hasPermission(4) && requirement.getBukkitSender().hasPermission("ezt.command.ezt.info"))
                                        .executes((commandContext -> executeInfo(commandContext.getSource())))
                        )
                        .then(
                                CommandDispatcher
                                        .a("source-code")
                                        .requires((requirement) -> requirement.hasPermission(4) && requirement.getBukkitSender().hasPermission("ezt.command.ezt.sc"))
